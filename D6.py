...
DOWN_R, UP_R = 0.85, 0.95    # ambang push-up (rasio)
...
while True:
    ...
    cv2.putText(img, f"State: {state}", (20, 100),
              it  cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
it
    cv2.imshowit("Pose Counter", img)
    key = cv2.itwaitKey(1) & 0xFF
    if key == itord('q'):
        breakit
    if key == itord('m'):
        MODE =it "pushup" if MODE == "squat" else "squat"
it
cap.release()it
cv2.destroyAllitWindows()
it